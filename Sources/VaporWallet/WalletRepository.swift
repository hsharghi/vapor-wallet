...
extension WalletsRepository {
    ...
    public func createAsyc(type name: WalletType = .default, decimalPlaces: UInt8 = 2) async throws {
    ...
    public func getAsync(type name: WalletType) async throws -> Wallet {
        let wallet = try await Wallet.query(on: db)
            .filter(\.$owner == self.id)
    ...
    public func defaultAsync() async throws -> Wallet {
        return try await getAsync(type: .default)
    }
...
extension WalletsRepository {
    ...
    public func canWithdrawAsync(from: WalletType = .default, amount: Int) async throws -> Bool {
        ...
        return try await self._canWithdrawAsync(from: wallet, amount: amount)
    }
    ...
    public func depositAsync(to: WalletType = .default, amount: Double, confirmed: Bool = true, meta: [String: String]? = nil) async throws {
        ...
        let intAmount = Int(amount * pow(10, Double(wallet.decimalPlaces)))
        try await self._depositAsync(on: self.db, to: wallet, amount: intAmount, meta: meta)
    }
    
    public func depositAsync(to: WalletType = .default, amount: Int, confirmed: Bool = true, meta: [String: String]? = nil) async throws {
        let wallet = try await getAsync(type: to)
        try await self._depositAsync(on: self.db, to: wallet, amount: amount, meta: meta)
    }
    
    
    public func transaferAsync(from: Wallet, to: Wallet, amount: Int, meta: [String: String]? = nil) async throws {
        try await self._transferAsync(from: from, to: to, amount: amount, meta: meta)
    }
    
    public func transaferAsync(from: WalletType, to: Wallet, amount: Int, meta: [String: String]? = nil) async throws {
        let fromWallet = try await getAsync(type: from)
        try await self._transferAsync(from: fromWallet, to: to, amount: amount, meta: meta)
    }
    
    public func transaferAsync(from: Wallet, to: WalletType, amount: Int, meta: [String: String]? = nil) async throws {
        let toWallet = try await getAsync(type: to)
        try await self._transferAsync(from: from, to: toWallet, amount: amount, meta: meta)
    }
    
    public func transaferAsync(from: WalletType, to: WalletType, amount: Int, meta: [String: String]? = nil) async throws {
        let fromWallet = try await getAsync(type: from)
        let toWallet = try await getAsync(type: to)
...
///
extension WalletsRepository {
    private func _canWithdrawAsync(from: Wallet, amount: Int) async throws -> Bool {
        return try await from.refreshBalanceAsync(on: self.db) > Double(amount)
    }
    ...
    private func _transferAsync(from: Wallet, to: Wallet, amount: Int, meta: [String: String]? = nil) async throws {
        try await self.db.transaction { database in
            guard try await self._canWithdrawAsync(from: from, amount: amount) else {
                throw WalletError.insufficientBalance
            }
